...
export async function parsePdf(title: string, pdfBytes: Uint8Array): Promise<Entry[][]> {
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    content.items.forEach((item) => {
      ...
      if (docType === DocType.ERTESITO) {
        const isNumberedHeading = new Array(10).fill(0).some((_v, i) => i + 1 >= blockCount) && item.str.startsWith(`${convertToRomanNumerals(i + 1)}. `);
        if (item.height === 10 && !item.str.includes(' FEJEZET') && !isNumberedHeading) {
          record = true;
          blockCount++;
          blocks.push([]);
  ...
  for (let block = 0; block < blockCount; block++) {
    ...
    docBlocks.push(entries.filter(e => e.name && !IGNORED_HEADINGS.some(ignored => e.id.trim().includes(ignored))))
  }
